...
# libpython3.1 excluded
sudo apt-get install --assume-yes apache2 apache2-mpm-worker apache2-utils apache2.2-bin apache2.2-common libapr1 libaprutil1 libaprutil1-dbd-sqlite3 python3-dev python3-minimal libapache2-mod-wsgi libaprutil1-ldap memcached python-cairo-dev python-django python-ldap python-memcache python-pysqlite2 sqlite3 erlang-os-mon erlang-snmp rabbitmq-server bzr expect ssh libapache2-mod-python python-setuptools
...
cp /tmp/graphite-settings.py /opt/graphite/webapp/graphite/settings.py
cp /tmp/graphite-aggregation-rules.conf /opt/graphite/conf/aggregation-rules.conf
sudo yes no | python manage.py syncdb
